...
def parse_option():
    ...
    parser.add_argument('--load_pretrain', type=str, default='load_pretrain', help='load_pretrain, no_pretrain')
    ...
    opt.save_path = "./save_upper_bound/save_{}_3M_incomplete_contrastive_{}/".format(opt.dataset, opt.load_pretrain)
...
def set_loader(opt):
    ...
    print("train data:")
    train_dataset = torch.utils.data.ConcatDataset([TrainA(complete=True), TrainB(complete=True)])
...
def load_single_modal(opt, layer_key):
    ...
    if opt.load_pretrain == "load_pretrain":
        if layer_key == 'acc_encoder.':
            opt.ckpt = './save_upper_bound/unimodal_pretrain/save_{}_acc_autoencoder/models/lr_0.0001_decay_0.0001_bsz_64/'.format(opt.dataset)
        elif layer_key == 'skeleton_encoder.':
            opt.ckpt = './save_upper_bound/unimodal_pretrain/save_{}_skeleton_autoencoder/models/lr_0.0001_decay_0.0001_bsz_64/'.format(opt.dataset)
        elif layer_key == 'gyro_encoder.':
            opt.ckpt = './save_upper_bound/unimodal_pretrain/save_{}_gyro_autoencoder/models/lr_0.0001_decay_0.0001_bsz_64/'.format(opt.dataset)
