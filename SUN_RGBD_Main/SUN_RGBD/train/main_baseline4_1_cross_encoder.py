...
def parse_option():
    ...
    opt.target_mod = 'semseg' if opt.dataset == 'train_A' else 'depth'
    return opt
...
def set_model(opt):
    if opt.dataset == "train_A":
        model = CrossModalGeneration(input_mod_channels=3, out_mod_channels=3) # Learn to generate semseg
    else:
        model = CrossModalGeneration(input_mod_channels=3, out_mod_channels=1) # Learn to generate depth
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, batched_data in enumerate(train_loader):
        ...
        loss = criterion(batched_data[opt.target_mod], output)
