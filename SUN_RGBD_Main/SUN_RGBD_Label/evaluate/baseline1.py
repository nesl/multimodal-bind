...
from models.models import IncompleteSupervised, SupervisedUnimodal
...
def set_folder(opt, trial_id):
    ...
    opt.save_path = './save_{}_baseline1/trial_{}/'.format(opt.dataset, trial_id)
...
def set_model(opt):
    depth_template = SupervisedUnimodal(in_channels=1)
    semseg_template = SupervisedUnimodal()
    semseg_template.load_state_dict(torch.load('../train/save_baseline1/save_train_A_autoencoder/models/lr_0.0001_decay_0.0001_bsz_64/last.pth')['model'])
    depth_template.load_state_dict(torch.load('../train/save_baseline1/save_train_B_autoencoder/models/lr_0.0001_decay_0.0001_bsz_64/last.pth')['model'])

    model = IncompleteSupervised()
    model.semseg_enc = semseg_template.encoder
    model.depth_enc = depth_template.encoder
