...
class IncompleteSupervised(nn.Module):
    ...
    def forward(self, x):
        ...
        semseg_embed = torch.reshape(semseg_embed, (bsz, -1))
        depth_embed = torch.reshape(depth_embed, (bsz, -1))
        return self.output_head(torch.cat( (semseg_embed, depth_embed ), dim=-1))
    

class IncompleteSupervisedPrompted(nn.Module):
    def __init__(self):
        super(IncompleteSupervisedPrompted, self).__init__()
        self.semseg_enc = Encoder(in_dim=3)
        self.depth_enc = Encoder(in_dim=1)
        self.output_head = nn.Sequential(
            nn.Linear(1536 * 2 + 3, 500),
            nn.ReLU(),
            nn.Linear(500, 6)
        )
    def forward(self, x):
        ...
        return self.output_head(torch.cat( (semseg_embed, depth_embed, x['mask'] ), dim=-1))
    


class SupervisedUnimodal(nn.Module):
    def __init__(self, in_channels=3, target_modality='img'):
    ...
    def forward(self, x):
        embed = self.encoder(x[self.target_modality])
        embed = torch.reshape(embed, (embed.shape[0], -1))
