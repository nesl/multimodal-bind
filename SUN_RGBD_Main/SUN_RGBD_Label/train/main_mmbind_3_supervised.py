...
from models.models import IncompleteSupervised, Incomplete_Contrastive_2M
...
def parse_option():
    ...
    opt.save_path = "./save_mmbind/save_{}_supervised_{}/".format(opt.dataset, opt.load_pretrain)
...
def set_loader(opt):
    ...
    if opt.dataset == "train_all_paired_AB":
        semseg_paired = PickleDataset('./save_mmbind/save_train_A')
        depth_paired = PickleDataset('./save_mmbind/save_train_B')
        train_A = TrainA()
        train_B = TrainB()
...
def set_model(opt):
    ...
    model_template = Incomplete_Contrastive_2M()
    model_template.load_state_dict(torch.load('./save_mmbind/save_train_all_paired_AB_contrastive_no_pretrain/models/lr_0.0001_decay_0.0001_bsz_64/last.pth')['model'])
    
    model.semseg_enc = model_template.semseg_enc
    model.depth_enc = model_template.depth_enc
