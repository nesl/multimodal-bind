...
def parse_option():
    ...
    opt.save_path = "./save_upper_bound/save_{}_supervised_{}/".format(opt.dataset, opt.load_pretrain)
...
def set_loader(opt):
    ...
    if opt.dataset == "train_AB":
        ...
        train_dataset = TrainA(complete=True)
    else:
        train_dataset = TrainB(complete=True)
...
def set_model(opt):
    ...
    model_template.load_state_dict(torch.load('./save_upper_bound/save_train_AB_3M_incomplete_contrastive_load_pretrain/models/lr_0.0001_decay_0.0001_bsz_64/last.pth')['model'])
