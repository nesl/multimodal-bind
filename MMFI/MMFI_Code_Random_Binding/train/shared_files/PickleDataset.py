...
# This dataset directly just reads from a directory, no config needed
class MMBind_PickleDataset(Dataset):

    def __init__(self, root_directory):
        self.data_paths = []
        for root, dirs, files in os.walk(root_directory):
            for file in files:
                self.data_paths.append(root + '/' + file)
    def __len__(self):
    ...
    def __getitem__(self, idx):
        ...
        data['modality'] = ['rgb', 'mmwave', 'depth']
...
def make_dataset(dataset_root, config, type='default'):
    if (type == 'default'):
        ...
        train_dataset = MMBind_PickleDataset(dataset_root)
        val_dataset = None
    else:
