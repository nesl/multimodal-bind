...
def parse_option():
    ...
    opt.save_path = "./save_baseline3/save_{}_mask_contrastive_{}/".format(opt.dataset, opt.load_pretrain)
...
def set_model(opt):
    ...
    model = model_lib.ContextDualContrastive()
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, batched_data in enumerate(train_loader):
        ...
        mask = torch.zeros((bsz, 3))
        i = 0
        for valid_mods in batched_data['modality']:
            mask[i] = torch.tensor(['depth' in valid_mods, 'mmwave' in valid_mods, 'rgb' in valid_mods])
            i += 1
        mask = mask.cuda()
        # Model returns three adapted embeddings (depth, mmwave, skeleton), input is entire dict
        depth_features, mmwave_features, skeleton_features = model(batched_data, mask)
