...
def set_loader(opt):
    ...
    train_dataset, _ = make_dataset('/mnt/ssd_8t/jason/Missing_Modalities/MMFI_Dataset', config_train)
    val_dataset, _ = make_dataset('/mnt/ssd_8t/jason/Missing_Modalities/MMFI_Dataset', config_val)
...
def set_model(opt):
    ...
    model_template_depth = model_lib.DualContrastiveModel()
    model_template_mmwave= model_lib.DualContrastiveModel()

    checkpoint = '../train/save_imagebind/save_train_AB_contrastive_depth_no_pretrain/models/lr_0.0001_decay_0.0001_bsz_32/last.pth'
    print(model_template_depth.load_state_dict(torch.load(checkpoint)['model'], strict=False))

    checkpoint = '../train/save_imagebind/save_train_AB_contrastive_mmwave_no_pretrain/models/lr_0.0001_decay_0.0001_bsz_32/last.pth'
    print(model_template_mmwave.load_state_dict(torch.load(checkpoint)['model'], strict=False))

    # Copy the model weights between the two models, TODO use pdb to verify that the weights are correctly loaded
    model.depth_encoder = model_template_depth.depth_encoder
    model.mmWave_encoder = model_template_mmwave.mmWave_encoder
