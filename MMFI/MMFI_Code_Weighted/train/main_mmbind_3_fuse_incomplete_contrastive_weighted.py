...
def collate_fn_padd(batch):
    ...
    for i in range(len(batch)):
        ...
        if ('similarity' not in batch[i].keys()):
            batch[i]['similarity'] = torch.FloatTensor([0.1])
    ...
    batch_data = {'modality': [batch[i]['modality'] for i in range(len(batch))],
                    ...
                    'similarity': torch.FloatTensor([sample['similarity'] for sample in batch])
                    }
...
def parse_option():
    ...
    opt.save_path = "./save_mmbind/save_{}_incomplete_{}/".format(opt.dataset, opt.load_pretrain)
...
def set_loader(opt):
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=opt.batch_size, collate_fn = collate_fn_padd, num_workers=opt.num_workers,
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, batched_data in enumerate(train_loader):
        ...
        with torch.no_grad():
            batched_data['input_depth'] += torch.randn(batched_data['input_depth'].shape) / 5
            batched_data['input_mmwave'] += torch.randn(batched_data['input_mmwave'].shape) / 5
            batched_data['input_rgb'] += torch.randn(batched_data['input_rgb'].shape) / 5
        

        # warm-up learning rate
        # warmup_learning_rate(opt, epoch, idx, len(train_loader), optimizer)
        similarity = batched_data['similarity']
        ...
        loss = criterion(features, similarity.cuda())
