...
def collate_fn_padd(batch):
    ...
    for i in range(len(batch)):
        ...
        if ('input_mmwave' not in batch[i].keys()):
            batch[i]['input_mmwave'] = torch.full((2, 297, 5), -1.1)
...
def parse_option():
    ...
    opt.save_path = "./save_imagebind/save_{}_contrastive_depth_{}/".format(opt.train_config, opt.load_pretrain)
...
def set_loader(opt):
    ...
    train_datasetA, _ = make_dataset('/mnt/ssd_8t/jason/Missing_Modalities/MMFI_Dataset', config_train)

    # with open('../Configs/config_train_B.yaml', 'r') as handle:
    #     config_train = yaml.load(handle, Loader=yaml.FullLoader)

    # train_datasetB, _ = make_dataset('/mnt/ssd_8t/jason/Missing_Modalities/MMFI_Dataset', config_train)
...
def set_model(opt):
    ...
    state_dict = torch.load('/home/jason/Documents/MMBind_Files/MMFI/MMFI_Code/MMFI_Code_Weighted/train/save_imagebind/save_train_AB_skeleton_AE/models/single_train_AB_lr_0.001_decay_0.0001_bsz_64/last.pth')['model']
    filtered_dict = {}
    for key, value in state_dict.items():
        if 'skeleton_encoder.' in key:
            filtered_dict[key.split('skeleton_encoder.')[-1]] = value
    model.skeleton_encoder.load_state_dict(filtered_dict)
    for param in model.skeleton_encoder.parameters():
