...
from models.model import SkeletonAE
...
def parse_option():
    ...
    opt.save_path = './save_imagebind/save_{}_{}_AE/'.format(opt.dataset, opt.modality)
...
def train(train_loader, model, optimizer, epoch, opt):
    ...
    for idx, batched_data in enumerate(train_loader):
        ...
        output = torch.reshape(output, batched_data['input_rgb'].shape)
        bsz = output.shape[0]
        loss = F.mse_loss(batched_data['input_rgb'].cuda(), output)
