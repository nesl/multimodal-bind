...
def set_model(opt):
    ...
    model_to_depth = model_lib.SkeletonToDepth()
    model_to_mmwave = model_lib.SkeletonToMMWave()
    # Use the skeletonAE() as a template to load weights in and then copy the weights
    model_to_depth.load_state_dict(torch.load('./save_baseline4/save_train_A_autoencoder/models/lr_0.0003_decay_0.0001_bsz_32/last.pth')['model'])
    model_to_mmwave.load_state_dict(torch.load('./save_baseline4/save_train_B_autoencoder/models/lr_0.0003_decay_0.0001_bsz_32/last.pth')['model'])

    model_to_depth = model_to_depth.cuda()
    model_to_mmwave = model_to_mmwave.cuda()
    return model_to_depth, model_to_mmwave
...
def main():
    ...
    model_to_depth, model_to_mmwave = set_model(opt)

    # Perform the validation for both datasets, paths_A, B used for creating new dataset
    gen_mmwave, label_A, paths_A = validate(train_loader_A, model_to_mmwave, opt)
    gen_depth, label_B, paths_B = validate(train_loader_B, model_to_depth, opt)

    save_paired_path = "./save_baseline4/train_paired_AB/"
    

    for sample_index in range(len(paths_A)):
        ...
        mmwave_data = np.reshape(gen_mmwave[sample_index], (30, 100, 5))
        mmwave_list = []
        for numpy_data in mmwave_data:
            mmwave_list.append(numpy_data)

        reference_data['input_mmwave'] = mmwave_list
        with open(relative_path_A + '/data.pickle', 'wb') as handle:
            pickle.dump(reference_data, handle)
   

    for sample_index in range(len(paths_B)):
        # Get the reference data dictionary and the select data dictionary. 
        # Goal is to create a new field ['input_X'] containing data from select
        ...
        # Add in the mmWave data
        reference_data['input_depth'] = np.squeeze(gen_depth[sample_index])
