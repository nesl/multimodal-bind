...
def main():
    ...
    for sample_index in range(paired_data_length):
        ...
        select_feature_indices = np.where(temp_similarity_vector > 0.9)[0]
        num_samples = 0
        for select_feature_index in select_feature_indices:
            total_pairs += 1
            ...
            if opt.reference_modality == "depth":
                ...
                with open(relative_path_A + '/data' + str(num_samples) + '.pickle', 'wb') as handle:
                ...
                with open(relative_path_B + '/data' + str(num_samples) + '.pickle', 'wb') as handle:
                    pickle.dump(reference_data, handle)
                num_samples += 1

    #np.save(save_paired_path + 'similarity_matrix.npy', similarity_matrix)
    # np.save(save_paired_path + 'label.npy', reference_label)

    print(similarity_record)
    print(correct_map, correct_map/total_pairs, total_pairs)
