...
from shared_files.PickleDataset import ProcessedDataset, make_dataset
import yaml
from models.model import mmWaveDepthContrastive
...
def parse_option():
    ...
    opt.save_path = "./save_upper_bound/save_{}_incomplete_{}/".format(opt.dataset, opt.load_pretrain)
...
def set_loader(opt):
    ...
    if (opt.dataset == 'train_A'):
        with open('../Configs/config_train_A_Complete.yaml', 'r') as handle:
        ...
        with open('../Configs/config_train_B_Complete.yaml', 'r') as handle:
            config_train = yaml.load(handle, Loader=yaml.FullLoader)
        train_dataset, _ = make_dataset('/mnt/ssd_8t/jason/MMFI_Dataset/', config_train)
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, batched_data in enumerate(train_loader):
        ...
        feature1, feature2 = model(batched_data)
