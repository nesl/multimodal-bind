...
class mmWaveEncoder(nn.Module):
    ...
    def forward(self, data):
        ...
        return torch.reshape(output_embeddings, (batch_size, -1))
...
class SkeletonAE(nn.Module):
    ...
    def forward(self, data):
        encoded = self.skeleton_encoder(data)
...
# Supervised model, we will load the encoders if using pretrained
class AllModsSupervised(nn.Module):
    def __init__(self, valid_mods):
        super(AllModsSupervised, self).__init__()
        self.mmWave_encoder = mmWaveEncoder()
        self.depth_encoder = DepthEncoder()
        self.skeleton_encoder = SkeletonEncoder()
        self.output_head = nn.Sequential(
        ...
        positions = torch.arange(0, 1000).unsqueeze_(1)
        self.pos_embeddings = torch.zeros(1000, d)
        denominators = torch.pow(10000, 2*torch.arange(0, d//2)/d) # 10000^(2i/d_model), i is the index of embedding
        self.pos_embeddings[:, 0::2] = torch.sin(positions/denominators) # sin(pos/10000^(2i/d_model))
        self.pos_embeddings[:, 1::2] = torch.cos(positions/denominators) # cos(pos/10000^(2i/d_model))
        self.depth_embed = nn.Parameter(torch.randn(64))
        self.mmwave_embed = nn.Parameter(torch.randn(64))
        self.skeleton_embed = nn.Parameter(torch.randn(64))


    def forward(self, data):
        combined_output = None
        if ('mmwave' in self.valid_mods):
            mmWave_output = self.mmWave_encoder(data)
            batch_size = mmWave_output.shape[0]
            mmWave_output = torch.reshape(mmWave_output, (batch_size, -1, 64))
            mmWave_output += self.mmwave_embed
            combined_output = mmWave_output
        if ('depth' in self.valid_mods):
            depth_output = self.depth_encoder(data)
            batch_size = depth_output.shape[0]
            depth_output = torch.reshape(depth_output, (batch_size, -1, 64))
            depth_output += self.depth_embed
            combined_output = depth_output if combined_output == None else torch.cat((combined_output, depth_output), dim=1)
        if ('skeleton' in self.valid_mods):
            skeleton_output = self.skeleton_encoder(data)
            batch_size = skeleton_output.shape[0]
            skeleton_output = torch.reshape(skeleton_output[:, :448], (batch_size, -1, 64))
            skeleton_output += self.skeleton_embed
            combined_output = skeleton_output if combined_output == None else torch.cat((combined_output, skeleton_output), dim=1)
       
        cls = repeat(self.cls, '1 1 n -> b 1 n', b=batch_size)
        # Concatenate, 61 (1 cls + 30 mmwave + 30 depth) tokens total
        combined_output = torch.cat((cls, combined_output), dim=1)
        combined_output += self.pos_embeddings[:combined_output.shape[1]].cuda()
        #combined_output = torch.cat((cls, depth_output), dim=1)
        cls_out = self.combine_features(combined_output)[:, 0] # Get only CLS and use for classification
        return self.output_head(cls_out)
...
class IncompleteSupervised(nn.Module):
    def __init__(self):
        super(IncompleteSupervised, self).__init__()
        self.mmWave_encoder = mmWaveEncoder()
        self.depth_encoder = DepthEncoder()
    ...
    def forward(self, data):
        depth_embed = self.depth_encoder(data)
        mmWave_embed = self.mmWave_encoder(data)
        result = torch.cat((depth_embed, mmWave_embed), dim=-1)
        return self.classifier_head(result)
    

class IncompleteSupervisedPrompted(nn.Module):
    def __init__(self):
        super(IncompleteSupervisedPrompted, self).__init__()
        self.mmWave_encoder = mmWaveEncoder()
        self.depth_encoder = DepthEncoder()
        self.classifier_head = nn.Sequential(
            nn.Linear(3844, 800),
