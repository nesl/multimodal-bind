...
def parse_option():
    ...
    parser.add_argument('--modality', action='store', type=str, nargs='*')
    # TODO changed
...
def set_model(opt):
    ...
    model = model_lib.IncompleteSupervised()
    model_template = model_lib.AllModsSupervised(opt.modality)
    model_template.load_state_dict(torch.load('../train/save_unimodal_depth/models/lr_0.0003_decay_0.0001_bsz_32/last.pth')['model'])
    model.depth_encoder = model_template.depth_encoder
    model_template.load_state_dict(torch.load('../train/save_unimodal_mmwave/models/lr_0.0003_decay_0.0001_bsz_32/last.pth')['model'])
