...
class WifiEncoder(nn.Module):
    def __init__(self):
        ...
        self.conv_layers  = nn.Sequential(
            nn.Conv2d(in_channels=3, out_channels=4, kernel_size=(10, 2)),
            nn.BatchNorm2d(4),
            nn.ReLU(),
            nn.MaxPool2d((2, 2)),
            nn.Conv2d(in_channels=4, out_channels=4, kernel_size=(5, 2)),
            nn.BatchNorm2d(4),
            nn.ReLU(),
            nn.MaxPool2d((2, 1)),
            nn.Conv2d(in_channels=4, out_channels=4, kernel_size=(5, 2)),
            ...
            nn.ReLU(),
            nn.MaxPool2d((2, 1)),
            nn.Conv2d(in_channels=4, out_channels=16, kernel_size=(5, 2))

        )
        self.GRU = nn.GRU(input_size=96, hidden_size=16, num_layers=2, batch_first=True, dropout=0.1)

    def forward(self, data):
        data = data['input_wifi-csi'].cuda()[:, 0:30]
        b_size, n_frames, channels, n1, n2 = data.shape
        data = torch.reshape(data, (-1, channels, n1, n2))
        conv_out = self.conv_layers(data)
        gru_in = torch.reshape(conv_out, (b_size, n_frames, -1))
        gru_out = self.GRU(gru_in)[0]
        return gru_out
    
class WifiDecoder(nn.Module):
    def __init__(self):
        super(WifiDecoder, self).__init__()
        self.conv_layers  = nn.Sequential(
            nn.ConvTranspose2d(in_channels=16, out_channels = 4, kernel_size=(5, 2), stride=(1, 1)),
            nn.ReLU(),
            nn.ConvTranspose2d(in_channels=4, out_channels = 4, kernel_size=(5, 2), stride=(2, 1)),
            nn.ReLU(),
            nn.ConvTranspose2d(in_channels=4, out_channels = 4, kernel_size=(10, 2), stride=(2, 1)),
            nn.ReLU(),
            nn.ConvTranspose2d(in_channels=4, out_channels = 3, kernel_size=(8, 4), stride=(2, 2)),
        )
        self.GRU = nn.GRU(input_size=16, hidden_size=96, num_layers=2, batch_first=True, dropout=0.1)

    def forward(self, data):
        b_size = data.shape[0]
        gru_out = self.GRU(data)[0]
        gru_out = torch.reshape(gru_out, (-1, 16, 6, 1))
        conv_out = self.conv_layers(gru_out)
        num_total, channels, h, w = conv_out.shape
        return torch.reshape(conv_out, (b_size, 30, channels, h, w))
    
class WiFiAE(nn.Module):
    def __init__(self):
        super(WiFiAE, self).__init__()
        self.enc = WifiEncoder()
        self.dec = WifiDecoder()
    def forward(self, data):
        embed = self.enc(data)
        return self.dec(embed)

    
if __name__ == '__main__':
    encoder = WifiEncoder().cuda()
    decoder = WifiDecoder().cuda()
    in_data = {'input_wifi-csi': torch.randn(64, 297, 3, 114, 10)}
    embedding = encoder(in_data)
