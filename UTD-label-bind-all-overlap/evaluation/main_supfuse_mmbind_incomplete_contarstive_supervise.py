...
def set_folder(opt, trial_id):
    ...
    source_path = "./save_{}mmbind_incomplete_supcon{}/trial_{}/".format(opt.dataset, opt.paired_data, trial_id)
...
def set_loader(opt):
    ...
    x_train_1, x_train_2, y_train, mask_train = data.load_data_incomplete_attach(opt.dataset)

    print("test data:")
    x_test_1, x_test_2, y_test, mask_test = data.load_data_incomplete_attach("test")

    train_dataset = data.Multimodal_incomplete_dataset(x_train_1, x_train_2, y_train, mask_train)
...
def load_multimodal_modal(opt):

    opt.ckpt = '../train/save_label_bind/save_mmbind_incomplete_supcon_more/trial_0/models/lr_0.0005_decay_0.0001_bsz_64/'
...
def set_model(opt):
    ...
    model.load_state_dict(load_multimodal_modal(opt))
