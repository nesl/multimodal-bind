...
def set_folder(opt, trial_id):
    ...
    source_path = "./save_label_bind/save_mmbind_incomplete_supcon{}/trial_{}/".format(opt.paired_data, trial_id)
...
def set_loader(opt):
    ...
    paired_mask_vector = np.zeros((x1_paired.shape[0], 2, 128))
    paired_mask_vector[:, 0, :] = 1.0
    paired_mask_vector[:, 1, :] = 1.0
    ...
    print("test data:")
    x_test_1, x_test_2, y_test, mask_test = data.load_data_incomplete("test")

    train_dataset = data.Multimodal_incomplete_dataset(x_train_1, x_train_2, y_train, mask_vector)
    test_dataset = data.Multimodal_incomplete_dataset(x_test_1, x_test_2,  y_test, mask_test)
