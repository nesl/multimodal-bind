...
class DualContrastiveIMUEncoder(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        norm_mod1_features = F.normalize(self.head_1(mod1_output), dim=1)
        norm_mod2_features = F.normalize(self.head_2(mod2_output), dim=1)
...
class ModEncoder(nn.Module):
    ...
    def forward(self, batched_data):
        ...
        return (
            nn.functional.normalize(acc_embed),
            nn.functional.normalize(gyro_embed),
            nn.functional.normalize(mag_embed),
        )
...
class SupervisedGyroMag(nn.Module):
    def __init__(self):
        ...
        self.gyro_encoder = SingleIMUEncoder("gyro")
        self.mag_encoder = SingleIMUEncoder("mag")
        self.output_head = nn.Sequential(nn.Linear(1920 * 2, 1920), nn.ReLU(), nn.Linear(1920, 7))
