...
def set_loader(opt):
    ...
    if opt.dataset == "train_A":
        train_dataset = data.Multimodal_dataset(x_train_2, x_train_1, y_train)
...
def main():
    ...
    for sample_index in range(len(label_list)):
        ...
        if opt.dataset == "train_A":
            np.save(save_paired_path + 'acc/{}.npy'.format(sample_index), original_data_list[sample_index])
            ...
            np.save(save_paired_path + 'acc/{}.npy'.format(sample_index), generate_data_list[sample_index])
            np.save(save_paired_path + 'label.npy', label_list)
