...
def set_loader(opt):
    ...
    print("train data:")
    x_train_1, x_train_2, y_train = data.load_original_data(opt.dataset)# [Acc, Skeleton] or [Acc, Gyro]
...
def set_model(opt):
    ...
    if opt.dataset == "train_A":
        model = MyUTDmodel_acc_to_skeleton(input_size=1)
    else:
        model = MyUTDmodel_acc_to_gyro(input_size=1)
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, (input_data1, input_data2, labels) in enumerate(train_loader):
        ...
        loss = criterion(input_data2, output)
