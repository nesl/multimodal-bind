...
def load_single_dataset(data_path):
	...
	if data_path == "train_A":
		x1, _, y = load_data_IMU(data_path)#acc
		x2, _ = load_data_skeleton(data_path)#skeleton

	elif data_path == "train_B":
		x1, x2, y = load_data_IMU(data_path)#acc, gyro
...
def load_upper_bound_all_dataset():

	x2_A, _ = load_data_skeleton("train_A")#skeleton
	x1_A, x3_A, y_A = load_data_IMU("train_A")#acc, gyro
	

	x2_B, _ = load_data_skeleton("train_B")#skeleton
	x1_B, x3_B, y_B = load_data_IMU("train_B")#acc, gyro
...
def load_upper_bound_all_incomplete_dataset():
	...
	x1_incomplete_1 = np.zeros_like(x1)# Acc missing; Acc, Skeleton, Gyro
	x2_incomplete_1 = x2
	x3_incomplete_1 = x3

	x2_incomplete_2 = np.zeros_like(x2)# Acc missing; Acc, Skeleton, Gyro
	x1_incomplete_2 = x1
	x3_incomplete_2 = x3

	x3_incomplete_3 = np.zeros_like(x3)# Acc missing; Acc, Skeleton, Gyro
	x1_incomplete_3 = x1
	x2_incomplete_3 = x2

	x1_all = np.vstack((x1_incomplete_1, x1_incomplete_2, x1_incomplete_3))
	x2_all = np.vstack((x2_incomplete_1, x2_incomplete_2, x2_incomplete_3))
	x3_all = np.vstack((x3_incomplete_1, x3_incomplete_2, x3_incomplete_3))
	y_all = np.hstack((y, y, y))
	...
	return x1_all, x2_all, x3_all, y_all
...
## load data for masked input
class Multimodal_incomplete_dataset_weighted():
	"""Build dataset from motion sensor data."""
	def __init__(self, x1, x2, x3, y, mask, similarity):
	...
	def __getitem__(self, idx):
		...
		return sensor_data1, sensor_data2, sensor_data3, activity_label, sensor_mask, activity_similarity
...
def load_data_incomplete(data_path):
	...
	if data_path == "train_A":
		x3 = np.zeros_like(x3)# Gyro missing; Acc, Skeleton, Gyro
		# x3 = np.random.randn(*(x3.shape))
		mask_vector[:, 0, :] = 1.0
		mask_vector[:, 1, :] = 1.0
	elif data_path == "train_B":
		x2 = np.zeros_like(x2)# Skeleton missing; Acc, Skeleton, Gyro
		# x1 = np.random.randn(*(x1.shape))
		mask_vector[:, 0, :] = 1.0
		mask_vector[:, 2, :] = 1.0
	else:
		x1 = np.zeros_like(x1)# Acc missing; Acc, Skeleton, Gyro
...
def load_original_data(data_path):
	...
	return load_x1, load_x2, y


def load_generated_data(data_path):

	folder_path = "./save_baseline4/{}/".format(data_path)

	x1 = []
	x2 = []
	# y = np.load(folder_path + "/label.npy")
	similarity = np.load(folder_path + "label.npy")
...
def load_all_generated_data():

	x1_A, x2_A, y_A = load_generated_data("train_A_generated_AB")
	x1_B, x2_B, y_B = load_generated_data("train_B_generated_AB")
