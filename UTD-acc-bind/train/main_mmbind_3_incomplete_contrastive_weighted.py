...
def parse_option():
    ...
    parser.add_argument('--dataset', type=str, default='train_all_paired_AB', 
                        choices=['train_skeleton_paired_AB', 'train_gyro_paired_AB', 'train_all_paired_AB'], help='dataset')
...
def set_loader(opt):
    ...
    paired_x_train_2, paired_x_train_3, similarity = data.load_all_paired_data()# skeleton, gyro
    paired_x_train_1 = np.zeros((paired_x_train_3.shape[0], 120, 3))#acc
    ...
    similarity = (np.log(similarity) / 2) + 1
    similarity = np.hstack((np.full(len(y_train) - len(similarity), 1),  similarity))
    #similarity = np.full(len(y_train), 1)
    print(x_train_1.shape, x_train_2.shape, x_train_3.shape, y_train.shape)

    train_dataset = data.Multimodal_incomplete_dataset_weighted(x_train_1, x_train_2, x_train_3, y_train, mask_vector, similarity)
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, (input_data1, input_data2, input_data3, label, mask, similarity) in enumerate(train_loader):
