...
def parse_option():
    ...
    parser.add_argument('--paired_dataset', type=str, default='acc_remain', help='acc_remain or acc_averaged')
...
def set_folder(opt, trial_id):
    ...
    opt.ckpt = '../label_bind/save_mmbind/save_label_bind_contrastive_{}/models/lr_0.0005_decay_0.0001_bsz_128/'.format(opt.load_pretrain)
    opt.save_path = './save_{}/supfuse_contrastive_add_label_pair_{}/trial_{}/'.format(opt.dataset, opt.train_ratio, trial_id)
...
def set_loader(opt, seed):

    #load originallabeled train and test data
    x1, x2, x3, y = data.load_data_IMU(opt.dataset)
    ...
    num_of_train = int(x1.shape[0] * opt.train_ratio)

    x1_train = X_train[0:num_of_train, 0:100, :]
    x2_train = X_train[0:num_of_train, 100:200, :]
    x3_train = X_train[0:num_of_train, 200:300, :]
    y_train_select = y_train[0:num_of_train]

    ##load paired data
    folder_path = "../label_bind/label_paired_data_Motionsense_Shoaib/{}/".format(opt.paired_dataset)
    x1_paired = np.load(folder_path + "acc.npy")
    x2_paired = np.load(folder_path + "gyro.npy")
    x3_paired = np.load(folder_path + "mag.npy")
    y_paired = np.load(folder_path + "label.npy")

    x1_stack_train = np.vstack((x1_train, x1_paired))
    x2_stack_train = np.vstack((x2_train, x2_paired))
    x3_stack_train = np.vstack((x3_train, x3_paired))
    y_stack_train = np.hstack((y_train_select, y_paired))


    print("train labeled data:")
    print(x1_stack_train.shape, x2_stack_train.shape, x3_stack_train.shape, y_stack_train.shape)

    x1_test = X_test[:, 0:100, :]
    x2_test = X_test[:, 100:200, :]
    x3_test = X_test[:, 200:300, :]
    print("test data:")
    print(x1_test.shape, x2_test.shape, x3_test.shape, y_test.shape)

    train_dataset = data.Multimodal_dataset(x1_stack_train, x2_stack_train, x3_stack_train, y_stack_train)
...
def set_model(opt):
    ...
    model.mag_encoder.load_state_dict(load_single_modal(opt, 'mag_encoder.'))
...
def main():
    ...
    for trial_id in range(opt.num_of_trial):
        ...
        for epoch in range(1, opt.epochs + 1):
            ...
            record_loss[epoch-1] = train_loss
