...
from shared_files.data_pre import PickleDataset, TrainA, TrainB

from models.models import Incomplete_Contrastive_3M
from shared_files.contrastive_design_3M_Weighted import FeatureConstructor, ConFusionLoss
...
def parse_option():
    ...
    parser.add_argument('--dataset', type=str, default='train_all_paired_AB', 
                        choices=['train_semseg_paired_AB', 'train_depth_paired_AB', 'train_all_paired_AB'], help='dataset')
    ...
    opt.save_path = "./save_mmbind/save_{}_incomplete_contrastive_weighted_{}/".format(opt.dataset, opt.load_pretrain)
...
def set_loader(opt):
    ...
    if opt.dataset == "train_all_paired_AB":
        semseg_paired = PickleDataset('./save_mmbind/train_semseg_paired_AB_test')
        depth_paired = PickleDataset('./save_mmbind/train_depth_paired_AB_test')
        train_A = TrainA()
        train_B = TrainB()
        train_dataset = torch.utils.data.ConcatDataset([semseg_paired, depth_paired, train_A, train_B])
    elif opt.dataset == "train_semseg_paired_AB":
        train_dataset = PickleDataset('./save_mmbind/train_semseg_paired_AB_test')
    elif opt.dataset == 'train_depth_paired_AB':
        train_dataset = PickleDataset('./save_mmbind/train_depth_paired_AB_test')
    else:
...
def load_single_modal(opt, layer_key):
    ...
    elif layer_key == 'acc_encoder.':
        opt.ckpt = './save_mmbind/save_train_AB_acc_AE/models/single_train_AB_lr_0.001_decay_0.0001_bsz_128/'
...
def set_model(opt):
    ...
    model = Incomplete_Contrastive_3M()
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, batched_data in enumerate(train_loader):
        ...
        feature_dict = model(batched_data)
        
        features = FeatureConstructor(feature_dict['img'], feature_dict['semseg'], feature_dict['depth'])

        loss = criterion(features, batched_data['similarity'])
