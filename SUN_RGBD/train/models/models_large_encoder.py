...
class Decoder(nn.Module):
    ...
    def __init__(self, in_dim=32, h_dim=32, out_dim=3, n_res_layers=10, res_h_dim=128):
        ...
        self.inverse_conv_stack = nn.Sequential(
            ...
            nn.ReLU(),
            # Add conv that keep size the same to increase number of parameters, check performance on AE
            nn.Conv2d(256, 256, kernel_size=3, padding='same'),
            nn.ReLU(),
            nn.ConvTranspose2d(256, 256, kernel_size=kernel,
                               stride=stride, padding=1),
            nn.ReLU(),
            # Add conv that keep size the same to increase number of parameters
            nn.Conv2d(256, 256, kernel_size=3, padding='same'),
            nn.ReLU(),
    ...
    def __init__(self, in_dim=3, h_dim=32, n_res_layers=10, res_h_dim=128):
        ...
        self.conv_stack = nn.Sequential(
            ...
            nn.Conv2d(100, 256, kernel_size=kernel,
                      stride=stride, padding=1),
            nn.ReLU(),
            nn.Conv2d(256, 256, kernel_size=kernel,
                      stride=stride, padding=1),
            nn.ReLU(),
...
class ImageAE(nn.Module):
    def __init__(self):
        ...
        self.enc = Encoder(h_dim=64)
        self.dec = Decoder(h_dim=64, in_dim=64)
