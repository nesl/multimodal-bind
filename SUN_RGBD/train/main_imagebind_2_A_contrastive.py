...
from models.models import Incomplete_Contrastive_3M, ImageAE
...
def set_model(opt):
    ...
    model_template = ImageAE()
    model_template.load_state_dict(torch.load('./save_imagebind/save_train_AB_acc_AE/models/single_train_AB_lr_0.0001_decay_0.0001_bsz_64/last.pth')['model'])
   
    model.image_enc = model_template.enc
    # Freeze the encoder for imagebind
    for param in model.image_enc.parameters():
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    # Mask is in order of ['img', 'semseg', 'depth']
    for idx, batched_data in enumerate(train_loader):
        ...
        # Mask shape will be b_size x 3
        
        features = FeatureConstructor(feature_dict['img'], feature_dict['semseg'])
