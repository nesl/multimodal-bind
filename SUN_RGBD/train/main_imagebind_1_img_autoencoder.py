...
def save_img(images_tensor):
    for i in range(images_tensor.size(0)):
        ...
        img = images_tensor[i].permute(1, 2, 0).cpu()  # Convert from [3, 640, 480] to [640, 480, 3]
        img = img * 255
        img[img > 255] = 255
        img[img < 0] = 0
        img = img.byte()  # Scale from [0,1] to [0,255] and convert to 8-bit
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, batched_data in enumerate(train_loader):
        ...
        loss = criterion(batched_data['img'], output)
