...
from models.models import DepthAE, SemSegAE
...
def parse_option():
    ...
    opt.current_mod = 'semseg' if opt.dataset == 'train_A' else 'depth'
    return opt
...
def save_img(images_tensor):
    for i in range(images_tensor.size(0)):
        ...
        img[img > 255] = 255
        img[img < 0] = 0
        img = torch.squeeze(img.byte())  # Scale from [0,1] to [0,255] and convert to 8-bit
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, batched_data in enumerate(train_loader):
        ...
        loss = criterion(batched_data[opt.current_mod], output)
