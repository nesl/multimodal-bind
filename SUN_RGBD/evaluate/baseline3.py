...
from models.models import SupervisedDepthSemseg, Masked_Incomplete_Contrastive_3M
...
def set_folder(opt, trial_id):
    ...
    opt.save_path = './save_{}_baseline3/trial_{}/'.format(opt.dataset, trial_id)
...
def set_model(opt):
    model_template = Masked_Incomplete_Contrastive_3M()
    model_template.load_state_dict(torch.load('../train/save_baseline3/save_train_AB_vector2_incomplete_contrastive_zero_no_pretrain/models/lr_0.0001_decay_0.0001_bsz_64/last.pth')['model'])

    model = SupervisedDepthSemseg()
    model.semseg_encoder = model_template.semseg_enc
    model.depth_encoder = model_template.depth_enc
