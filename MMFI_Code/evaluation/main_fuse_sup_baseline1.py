...
from models.model import mmWaveDepthSupervised
...
# Evaluates the unsupervised autoencoders trained in main_baselin1_separate_autoencoders

# Collate fn is used by the dataloader to apply preprocessing to data, this pads the mmwave sizes
...
def parse_option():
    ...
    # Point to the appropriate configuration file (C is used for fine tuning) while val is used for testing
...
def set_loader(opt):
    ...
    # Open the appropriate YAML files provided as command line arguments
...
# Load model weights, modify the ckpt path to accomodate different configurations
def load_single_modal(opt, dataset):

    opt.ckpt = '../train/save_baseline1/save_{}_autoencoder/models/lr_0.0003_decay_0.0001_bsz_32/'.format(dataset)
    ckpt_path = opt.ckpt + 'last.pth'
    ckpt = torch.load(ckpt_path, map_location='cpu')
    state_dict = ckpt['model']
    return state_dict

# Create a mmWaveDepthSupervised model with depth encoder, mmWave encoder, TE for fusion
# Load the appropriate pretrained weights into the depth encoder and mmWave encoder
def set_model(opt):

    model = mmWaveDepthSupervised()
    ...
    model.depth_encoder.load_state_dict(load_single_modal(opt, 'train_A'))
    model.mmWave_encoder.load_state_dict(load_single_modal(opt, 'train_B'))
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    # batched_data is a dictionary containing 
    #   'action': gt actions
    #   'input_X': batch_size x 30 x ... modality data
    for idx, batched_data in enumerate(train_loader):
        ...
        # Actions are the string ground truths, i.e 'A01'
        actions = batched_data['action']
        # Convert actions to integer indices
        labels = torch.tensor([int(item[1:]) - 1 for item in actions]).cuda()
        output = model(batched_data) # model directly takes the entire batched_data dictionary
