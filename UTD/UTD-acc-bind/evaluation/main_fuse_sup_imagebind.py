...
from models.fuse_3M_acc_skeleton_gyro import MyUTDmodel_3M, MyUTDmodel_3M_contrastive
...
def set_folder(opt, trial_id):
    ...
    opt.ckpt = '../train/save_imagebind/save_train_A_dual_contrastive_zero_no_pretrain/models/lr_0.0001_decay_0.0001_bsz_64/'

    # set the path according to the environment
    opt.save_path = './save_{}imagebind/trial_{}/'.format(opt.dataset, trial_id)
...
def set_model(opt):
    ...
    temp_modelA = MyUTDmodel_3M_contrastive(input_size=1) # Gyro Missing
    temp_modelB = MyUTDmodel_3M_contrastive(input_size=1) # Skeleton Missing

    temp_modelA.load_state_dict(torch.load('../train/save_imagebind/save_train_A_dual_contrastive_zero_no_pretrain/models/lr_0.0001_decay_0.0001_bsz_64/last.pth')['model'])
    temp_modelB.load_state_dict(torch.load('../train/save_imagebind/save_train_B_dual_contrastive_zero_no_pretrain/models/lr_0.0001_decay_0.0001_bsz_64/last.pth')['model'])

    model = MyUTDmodel_3M(input_size=1, num_classes=opt.num_class)
    model.skeleton_encoder = temp_modelA.skeleton_encoder
    model.gyro_encoder = temp_modelB.gyro_encoder
    model.acc_encoder = temp_modelA.acc_encoder
