...
from models.fuse_3M_acc_skeleton_gyro import MyUTDmodel_3M_contrastive_mask2
...
def parse_option():
    ...
    opt.save_path = "./save_baseline3/save_{}_vector2_incomplete_contrastive_zero_{}/".format(opt.dataset, opt.load_pretrain)#attach the vector [1,1,0] to features
...
def set_loader(opt):
    ...
    if opt.dataset == "train_AB":
        x_train_1, x_train_2, x_train_3, y_train, mask_train = data.load_all_data_incomplete_attach()
    else:
        x_train_1, x_train_2, x_train_3, y_train, mask_train = data.load_data_incomplete_attach(opt.dataset)
...
def set_model(opt):
    ...
    model = MyUTDmodel_3M_contrastive_mask2(input_size=1)
    ...
    if opt.load_pretrain == "load_pretrain":
        model.acc_encoder.load_state_dict(load_single_modal(opt, 'skeleton_encoder.'))
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, (input_data1, input_data2, input_data3, label, mask) in enumerate(train_loader):
        ...
        # print(input_data1[0].mean(), input_data2[0].mean(), input_data3[0].mean(), mask[0])

        feature1, feature2, feature3 = model(input_data1, input_data2, input_data3, mask)

        # print(feature1[0].mean(), feature2[0].mean(), feature3[0].mean())
