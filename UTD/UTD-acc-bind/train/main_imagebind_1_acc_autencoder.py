...
from models.single_modality import MyUTDmodel_acc_AE
...
def parse_option():
    ...
    parser.add_argument('--dataset', type=str, default='train_AB',
                        choices=['train_AB'], help='dataset')
    parser.add_argument('--common_modality', type=str, default='acc',
                        choices=['acc', 'skeleton', 'gyro'], help='common_modality')
    ...
    opt.save_path = './save_imagebind/save_{}_{}_AE/'.format(opt.dataset, opt.common_modality)
...
def set_loader(opt):
    ...
    single_x_train, _, y_train = data.load_all_data_IMU()# acc as common modality

    train_dataset = data.Unimodal_dataset(single_x_train, y_train)
