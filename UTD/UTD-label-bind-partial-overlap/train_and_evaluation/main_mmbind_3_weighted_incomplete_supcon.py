...
from models.fuse_2M_skeleton_gyro import MyUTDmodel_2M_plus_contrastive
from shared_files.contrastive_weighted_design_2M import FeatureConstructor, ConFusionLoss
...
def parse_option():
    ...
    parser.add_argument('--weight_decay', type=float, default=1e-4,
                        help='weight decay')
    parser.add_argument('--lr_decay_epochs', type=str, default='50, 100,150,200',
    ...
    parser.add_argument('--paired_data', type=str, default='_more', help='blank or _more')
...
def set_folder(opt, trial_id):
    ...
    source_path = "./save_label_bind/save_mmbind_incomplete_weighted_supcon_more_new_reweight/trial_{}/".format(trial_id)
...
def set_loader(opt):
    ...
    # ##load paired data
    # folder_path = "./save_mmbind{}_label_paired_data/".format(opt.paired_data)
    # x1_paired = np.load(folder_path + "skeleton.npy")
    # x2_paired = np.load(folder_path + "gyro.npy")
    # y_paired = np.load(folder_path + "label.npy")
    # similarity_paired = np.load(folder_path + "similarity.npy")

    original_x_train_1, original_x_train_2, original_y_train, original_mask_vector = data.load_all_data_incomplete()
    folder_path = "./save_mmbind_more_label_paired_data_reweight/"
    x1_paired = np.load(folder_path + "skeleton.npy")
    x2_paired = np.load(folder_path + "gyro.npy")
    y_paired = np.load(folder_path + "label.npy")
    similarity_paired = np.load(folder_path + "similarity.npy")
    ...
    # similarity_train = np.hstack((np.ones(original_y_train.shape[0]), similarity_paired))
    similarity_paired = (np.log(similarity_paired) / 2) + 1
    similarity_train = np.hstack((np.ones(original_y_train.shape[0]),  similarity_paired))

    print("test data:")
    x_test_1, x_test_2, y_test = data.load_single_dataset("test")

    train_dataset = data.Multimodal_paired_dataset(x_train_1, x_train_2, y_train, similarity_train)
...
def set_model(opt):
    ...
    model = MyUTDmodel_2M_plus_contrastive(input_size=1, num_classes=opt.num_class)
...
def train(train_loader, model, criterion1, criterion2, optimizer, epoch, opt):
    ...
    for idx, (input_data1, input_data2, labels, similarity) in enumerate(train_loader):
        ...
        loss2 = criterion2(features, similarity)
        loss = loss1 + loss2
