...
from models.single_modality import MyUTDmodel_acc, MyUTDmodel_gyro, MyUTDmodel_skeleton
...
def set_folder(opt, trial_id):
    ...
    opt.save_path = './save_{}single_{}_upper_bound/trial_{}/'.format(opt.dataset, opt.modality, trial_id)
...
def set_loader(opt):
    ...
    elif opt.modality == "gyro":
        x_train = x_train_label_2
    elif opt.modality == "skeleton":
        x_train = x_train_label_3

    print("test data:")
    x_test_1, x_test_2, y_test = data.load_data_IMU("test")
    x_test_3, _ = data.load_data_skeleton("test")
...
def set_model(opt):
    ...
    if opt.modality == "acc":
        model = MyUTDmodel_acc(input_size=1, num_classes=opt.num_class)
    elif opt.modality == "gyro":
        model = MyUTDmodel_gyro(input_size=1, num_classes=opt.num_class)
    elif opt.modality == "skeleton":
        model = MyUTDmodel_skeleton(input_size=1, num_classes=opt.num_class)
